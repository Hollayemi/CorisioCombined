...
export default function Verify({ route }: { route: VerifyProp }) {
    ...
    const handleResendOtp = async () => {
        try {
            await resendOtp({ email, accountType: "staff", action: { account: "staff" } }).unwrap();
    ...
    const handleVerifyOtp = async (otp: string, type: string) => {
        try {
            ...
            if (type === "createAccount") {
                await verifyOtp({ otp, email, returnToken: true }).unwrap()
                router.push("/auth/Created");
            } else if (type === "updatePassword") {
                const result = await verifyOtp({ otp, email, returnToken: true }).unwrap()
                router.push({ pathname: "/auth/UpdatePassword", params: { token: result.token, email } });
            }
        } catch (error) {
        } finally {
            setOTP(["", "", "", "", "", ""]);
    ...
    return (
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <SafeAreaView className="flex-1 bg-white dark:bg-gray-900 px-5 items-center">
                <View className="w-full mt-16" style={{ marginTop: 70 }}>
                    <PageHeader
                        title="Verify your email"
                        subtitle="A verification code has been sent to your email, check it and use the code to verify your account."
                    />
                </View>
                ...
                <Text className="text-sm mt-4 font-medium text-gray-800 dark:text-gray-200 text-center">
